		...
		private void BuildUI()
		{
			...
			try
			{
				...
				var hasmyeffect = pi.ClientState.Actors
					...
					{
						if (pi.Framework.Gui.WorldToScreen(new SharpDX.Vector3(actor.Position.X, actor.Position.Z-this.configuration.z位置, actor.Position.Y), out SharpDX.Vector2 pos))
                    {
                        ...
                        var screenPosO = new Vector2(pos.X - this.configuration.x位置, pos.Y);
							var effects = actor.StatusEffects.Where(i => i.OwnerId == localPlayerActorId).Select((effect, i) => (effect.EffectId, i));
