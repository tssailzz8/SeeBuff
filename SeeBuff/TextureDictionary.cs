...
namespace QoLBar
{
	public class TextureDictionary : ConcurrentDictionary<int, TextureWrap>, IDisposable
	{
		...
		{
			var iconTex = useHR ? GetHRIcon(icon) : External.Data.GetIcon((uint)icon);
		...
		{
			var iconTex = External.Data.GetFile<Lumina.Data.Files.TexFile>(path);
			return (iconTex == null) ? null : LoadTextureWrapSquare(iconTex);
		});
		...
		private void LoadImage(int iconSlot, string path, bool overwrite) => LoadTextureWrap(iconSlot, overwrite, true, () => Plugin.pi.UiBuilder.LoadImage(path));

		private Lumina.Data.Files.TexFile GetHRIcon(int icon)
		{
			var path = $"ui/icon/{icon / 1000 * 1000:000000}/{icon:000000}_hr1.tex";
			return External.Data.GetFile<Lumina.Data.Files.TexFile>(path) ?? External.Data.GetIcon((uint)icon);
		}
		...
		private TextureWrap LoadTextureWrapSquare(Lumina.Data.Files.TexFile tex)
		{
			...
			if (tex.Header.Width > tex.Header.Height)
			{
				...
				return Plugin.pi.UiBuilder.LoadImageRaw(newData, tex.Header.Width, tex.Header.Width, 4);
			}
			else if (tex.Header.Width < tex.Header.Height)
			{
				...
				return Plugin.pi.UiBuilder.LoadImageRaw(newData, tex.Header.Height, tex.Header.Height, 4);
			}
			else
			{
				return Plugin.pi.UiBuilder.LoadImageRaw(imageData, tex.Header.Width, tex.Header.Height, 4);
			}
		}
	...
	public static class Extensions
	{
		...
		{
			var rgba = TexFileExtensions.GetRgbaImageData(tex);
