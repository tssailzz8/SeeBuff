...
namespace QoLBar
{
	public class TextureDictionary : ConcurrentDictionary<int, TextureWrap>, IDisposable
	{
		...
		{
			var iconTex = useHR ? GetHRIcon(icon) : Plugin.pi.Data.GetIcon(icon);
		...
		{
			var iconTex = Plugin.pi.Data.GetFile<Lumina.Data.Files.TexFile>(path);
			return (iconTex == null) ? null : LoadTextureWrapSquare(iconTex);
		});
		...
		private Lumina.Data.Files.TexFile GetHRIcon(int icon)
		{
			...
			return Plugin.pi.Data.GetFile<Lumina.Data.Files.TexFile>(path) ?? Plugin.pi.Data.GetIcon(icon);
		}
